...
def create_feature_space(sentences):
	...
	for i in range(0,len(sentences)):
		...
		for j in range(0, len(k)):
			k[j] = st.stem(k[j])
			if k[j] not in a and k[j] not in stop:
				a[k[j]]=len(a)
	return a
...
#	m1 = train(prob, param)
	#print 'get here'
	#save_model('libsvm_SVC_stem_emoticons_b.model', m1)
	m1 = gnb.fit(vectors, labels)
...
def Testing(directory,m1,feature_space):
	...
	r = m1.predict(vectors)
	
	print "Number of mislabeled points : %d" % (labels != r).sum()
	#print m
	return 
