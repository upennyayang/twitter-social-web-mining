...
def vectorize(feature_space, sentence,weight):
	...
	for key in feature_space:
		if key in k:
			result[feature_space[key]]+=weight[key]
		else:
			result[feature_space[key]]=0
	return result
	

def Training(directory,n):
	...
	[feature_space,w] = create_feature_space_IG(n)
	#print feature_space
	# try:
	...
	for f in os.listdir(directory):
		...
		for line_num, line in enumerate(of.readlines()):
			vector = vectorize(feature_space,line,w)
	...
	param = parameter('-s 0')
	#print 'get here'
	m1 = train(labels,vectors, param)
	#print 'get here'
	#save_model('libsvm_SVC_IG.model', m1)
	return [m1,feature_space,w]

def Testing(directory,m1,feature_space,w):
	vectors = []
	labels = []
	count = 0
	for f in os.listdir(directory):
...
#createWordCount(Training_Path,"/Users/oliverfengpet/Dropbox/TwitterAffect/IG/")
for i in range(2,21):
	print 'The number of feature is '+ str(i*500)
	[m, fs,w] = Training(Training_Path,i*500)
	Testing(Testing_Path,m,fs,w)
