...
label_map ={'Training_Happy.txt':0,'Training_Angry.txt':0,'Training_Sad.txt':0,'Training_Ashamed.txt':0,'Training_Afraid.txt':1}
test_label_map ={'Test_Happy.txt':0,'Test_Angry.txt':0,'Test_Sad.txt':0,'Test_Ashamed.txt':0,'Test_Afraid.txt':1}
...
def Training(directory):
	...
	#print 'get here'
	#svm_save_model('libsvm_SVC_stem_emoticons_Afraid.model', m1)
...
def Testing(directory,m1,feature_space):
	...
	err_f = open('err_afraid.txt','w+')
	
	for i in range(0,len(m)):
		if(m[i]!=labels[i]):
			err_f.write(str(p_vals[1])+'	'+lines[i])
