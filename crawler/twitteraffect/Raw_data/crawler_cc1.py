...
POST_PARAMS = {'include_entities': 0,
               ...
               'stall_warning': 'true',
               #'track': ':),:-),:D,:-D'}
               'track': '#ashamed,#sorrowful,#remorseful,#defamed,#worthless,#disgraced,#dishonored, #mortified,#admonished,#apologetic,#unworthy,#sneaky,#guilty,#embarrassed,#secretive,#bashful,#regretful,#uncomfortable,#pitied,#silly'}

TWEETS_PER_FILE=1000
Des_Folder_Name='PleaseChange'
...
def result_to_file(copy_results):
    ...
    tsp=datetime.now()
    data_dir=os.sep.join(['..','raw_data',Des_Folder_Name])
...
if __name__ == '__main__':
    if len(sys.argv)!=2:
        print 'Usage: python crawler_cc.py folderName'
        sys.exit()
    #           POST_PARAMS['track']=sys.argv[1]
    #       print (sys.argv[1])
    Des_Folder_Name=str(sys.argv[1])
    print POST_PARAMS['track']
    print Des_Folder_Name
    ts = TwitterStream()
    ts.setup_connection()
    ts.start()
